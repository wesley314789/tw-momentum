...
* **價格未還原權值**。交易所每日行情仍保留原始價。長期回測另用
  `scripts/corporate_actions.py` 的官方除權息參考價近似校正 OHLC；未能解釋的跳價繼續
  排除並計數。改校正方式時要重算回測名單，不要把被排除的交易當成零損益。
* **美股歷史宇宙**。`data/us_universe/YYYY-MM-DD.csv.gz` 只保存收盤後 18 小時內實際
  觀察的名單。更早的 `--backfill` 用當前 Nasdaq 名單回推，標成
  `reconstructed_current`，有生存者偏差；沒有當日快照就不能改標籤。
