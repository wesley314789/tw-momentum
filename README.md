...
2. **回補歷史資料**(只需做一次):
   - 到 [FinMind](https://finmindtrade.com/) 免費註冊拿 token
   - 本機執行:
     ```bash
     pip install pandas requests
     FINMIND_TOKEN=你的token python scripts/update_data.py --backfill
     ```
   - 約 280 次 API 呼叫,免費額度內可完成(跑 6–10 分鐘)
...
- 歷史回補:FinMind `TaiwanStockPrice`(免費 token,只用一次)
