...
3. 總市值 ≥ 20 億
4. 成交值 ≥ 5,000 萬
5. 近 21 個交易日漲幅 − 同期加權指數漲幅 > 10 個百分點
...
| 日期 | 大盤 21 日漲幅 | 舊(漲幅 > 20%) | 新(超額 > 10) |
|---|---|---|---|
| 2026-05-04 | +28.3% | 290 | 159 |
| 2026-03-31 | −9.6% | 54 | 170 |
| 2026-07-31 | −8.3% | **8** | 66 |

大盤一個月漲 28% 時,舊條件把跟著大盤水漲船高的都算進去;大盤跌 8% 時,舊條件幾乎歸零,看不出誰在逆勢撐住。新條件選的是「跑贏市場」,不是「剛好在漲的市場裡」。兩者在重疊的 279 天相關係數 0.82 —— 大方向一致,但極端行情時差到 100 檔以上。

實作上兩件事要注意:

* **大盤漲幅用這檔股票自己的視窗算**(它 21 根 K 棒前那天到今天)。中間停牌過的個股,這個起點會早於「全市場 21 個交易日前」,要比就得比同一段。
...
要知道的副作用:這段期間 S&P 500 一個月漲幅多在 ±3% 以內,新門檻實際上約等於「漲超過 10%」,比舊的 20% 寬很多,通過檔數明顯變多(2026-06-30 舊 260 檔、新 682 檔)。這是「超額 > 10」在盤整市的自然結果,不是 bug;大盤單月大漲時則會反過來比舊條件嚴。
